...
| Only the separately named FLAME 2023 Open model is CC-BY-4.0 and permits commercial use with attribution and published use restrictions; standard/older releases are non-commercial. | [FLAME model licenses](https://flame.is.tue.mpg.de/modellicense.html), [official FLAME release page](https://flame.is.tue.mpg.de/) | First-party sources checked 2026-09-21. The download requires sign-in. Registered scan data, landmark assets and the separate texture model are not covered by this model grant. |
