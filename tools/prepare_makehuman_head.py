...
def extract_head(text, minimum_y=5.5):
    ...
    for line in text.splitlines():
        ...
        if a[0]=='v':vertices.append([float(x) for x in a[1:4]])
            ...
            if len(indices) not in (3,4):raise ValueError('only triangle/quad body faces supported')
...
def main():
    ...
    v,f,used=extract_head((args.assets/'base.obj').read_text())
    ...
    report=dict(status='UNVERIFIED',asset=lock,vertices=len(v),triangles=len(f),
                ...
                scope='Unfitted generic head/neck template. Open neck crop; no eyeballs or separate teeth imported.')
