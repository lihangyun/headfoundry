...
The first milestone is deliberately narrow: prove that camera projection can be recovered and measured reliably. Later user authorization permits unaccepted experimental geometry while that gate remains failed; it does not waive acceptance. The repository contains a normalized-DLT reference, fail-closed rights/image-input manifests, a commercial-VGGT output adapter, and camera quality gates. The current five references were independently generated or composited, so their historical camera scores are image-group diagnostics, not physical capture validation.

## Current status

- Experiment 0196 replaces only the local lip section using explicitly closed anterior strips from the pinned Apache-2.0 GNM neutral model. A concrete interpolated-rim discontinuity is corrected in one exact-rim rerun, but actual five-view renders flatten the lips and both profile diagnostics worsen. Both candidates remain `REJECT`; no head/camera default changes. Stop fixed-rim section transfer and screen a coherent lip-to-philtrum/chin surface next. Hair remains excluded.
- Experiment 0195 creates two actual structured-lip head candidates from newly reviewed native image cues. Both pass mesh guards and five exact-camera renders, and lower-lip relief becomes clearer, but unnatural seam/puckering and shelf-like profiles remain. Both are `REJECT`; no default head change. Stop additive ribbon tuning and require explicit donor closure/exterior selection before a local anatomical lip-surface replacement.
...
- Experiments 0189–0190 replace local silhouette motion with a coherent, licensed CC0 3D head-control prior. On a predeclared disjoint twelve-head synthetic split, mean bilateral side-edge and true 3D errors improve, but only 5/12 cases pass the joint side/3D/front/mesh gate. The frozen method remains `REJECT` for reliable head recovery and has **not** changed the subject mesh; local five-photo likeness is `UNVERIFIED`.
- Experiment 0188 runs a 12-case fixed-3D, commercially permitted synthetic head benchmark: only front and ±30° masks fit the template, while ±90° images and true meshes are held for evaluation. The existing local contour fitter improves both side edges and 3D RMSE in just 1/12 cases (minutely); average 3D error and front training IoU worsen, and one mesh fails safety. This head-shape route is `REJECT`, not a surrogate claim about the independently generated five-photo subject.
- Experiment 0187 tests removing only the current licensed chin-width-decrease control, after discovering that a broad skin/neck edge had been mistaken for the jaw. All bounded trials are geometrically safe, but frontal jaw diagnostics worsen slightly, left/right side effects oppose, and exact five-view renders show no confident visible gain. The ablation is `REJECT`; current head unchanged.
- Experiment 0186 establishes a head-only five-view clay/photo baseline after the source correction, using the already licensed generic-eye candidate with the unchanged head and cameras. Rights and exact Blender projection replay pass (worst maximum 0.000116 px); eye-surface completeness remains `PARTIAL_SUCCESS`, while recognizable complete-head likeness/default promotion is `REJECT`. Hair is outside the current scope.
- Current priority is the **head mesh only**: front, both side profiles, ears, jaw and neck; hair experiments are paused. The user confirmed the five local references were independently generated or composited, not a common rigid capture. They are useful visual targets but cannot establish one true camera system or physical subject identity. Experiment 0185 found no new <=3 px three-view material track; its prior 7.374 px p95 remains a failed image-group diagnostic, not a measured real-camera error. No camera was changed and no head quality is accepted.
- Experiment 0184 grafts only the pinned Apache-2.0 GNM Head neck cross-section changes below the existing open ring, leaving the subject face/cameras/hair fixed. Exact five-view renders replace the immediate short stump but make a posterior shoulder wedge, flat lower cut and collar overlap; visual/default promotion is `REJECT` despite valid topology.
- Experiment 0183 tests a closed posterior hair volume, eleven curved bun locks and two fixed-mesh ear-color fills independently. Posterior side-mask IoU improves without new ear occlusion, but real renders still show a hard dark nape wedge; locks show a striped fan on the old ball/stalk; ear fill improves one source view but leaves multiview patches. Numerical local gains are `PARTIAL_SUCCESS`, all visual/default candidates `REJECT`. None is combined or promoted.
- Experiment 0182 isolates the missing ear-to-nape hair in exact side-view renders: much of it lies beyond any rendered surface, so recoloring cannot repair the contour. A thin posterior layer reduces missed hair but creates a dark vertical neck patch and overcovers the ears; two bounded bun variants still look like a ball on a thin stalk. All three are `REJECT` after five-view photo inspection. The next geometry needs naturally swept posterior hair and a gathered bun while protecting ear visibility, not another flat patch or local smooth deformation.
- Experiment 0181 replaces the failed local-normal hair deformation with a smooth 40-control radial field on the same connected hair mesh. Exact five-view hair-mask IoU rises 0.685→0.717 / 0.584→0.627 / 0.606→0.640 / 0.638→0.647 / 0.623→0.634, with at most 0.0954 model-unit displacement and no face reversals. This is `PARTIAL_SUCCESS` for silhouette only; the private photo comparison still shows a helmet, ball bun and broken ear/nape transition, so visual/default promotion is `REJECT`.
- Experiment 0180 fits the first 30 commercially permitted GNM Head identity directions to front/±30° anchors, then alternate rows of both pure profiles. The exact same-camera, same-row 1254 px raster comparison loses to the current head: left90 6.53 vs 4.24 px and right90 7.00 vs 4.94 px; withheld rows lose too. Five-view clay inspection remains generic despite better ear/neck completeness. This candidate/default promotion is `REJECT`; do not substitute a legal generic prior for observed subject likeness.
- Experiment 0179 locally screens the official Apache-2.0 GNM Head mean and its 253-dimensional identity basis without adding it to the product. The mean has better generic ear/neck anatomy but is not this person after a two-parameter vertical fit; direct replacement is `REJECT`, subject identity fitting is `UNVERIFIED`. This is a licensed candidate prior, not a visual improvement or camera acceptance.
- Experiment 0178 optimizes six smooth hair-shape coefficients on four views and checks the exact five-view mesh. Left/right pure-profile hair-mask IoU rises 0.606→0.636 / 0.623→0.630, while front falls 0.685→0.682. A local photo/old/new contact sheet still shows the helmet crown, spherical bun and ear/nape mismatch. Mask fitting is `PARTIAL_SUCCESS`; visual/default promotion is `REJECT`. Stop this scalar family and require a different shape representation.
- Experiment 0177 produces one locally connected, closed hair surface via Blender solidification and voxel union (63,844 vertices, 127,708 triangles, no boundary/non-manifold edges). Its five-view silhouettes remain near the rejected 0176 version, and photo-projected hair color helps a fitted left profile but not the complete right-profile hair area (L1 0.2188 flat vs 0.2194 colored). The representation is `PARTIAL_SUCCESS`; real full-head appearance/default promotion is `REJECT`. Stop color tuning and change the posterior/crown/bun geometry under side-profile protection.
- Experiment 0176 diagnoses hair v2 undercoverage on visible scalp, selects a three-parameter direction-dependent hairline on four views, and precisely clips cap triangles at that boundary. Actual five-view renders remove the jagged edge and improve side hair-mask IoU (left90 0.370→0.609; right90 0.422→0.626), but reveal a uniform dome, incomplete ear transition and primitive bun/connector. The scalp-coverage direction is `PARTIAL_SUCCESS`; complete hair/default promotion is `REJECT`. The right90 boundary was excluded from selection, but the older head/cameras/bun had already used it; this is not independent reconstruction validation.
- Experiment 0175 projects source-visible skin color onto the unchanged continuous head mesh, excluding right90 from color sampling. The face is visibly more coherent in all five local renders than the prior point-splat diagnostic, including right90, but the head/cameras had already used all five photos; this is not a reconstruction holdout. An actual render with the previously rejected hair geometry exposes a hard hairline and disconnected bun (five-view hair IoU 0.342–0.485). Facial appearance is `PARTIAL_SUCCESS`; full head, hair and default promotion remain `REJECT`. Next solve the continuous scalp-to-bun surface and independently validate the camera/shape.
- Experiment 0174 runs a local fixed-geometry Gaussian appearance fit on four views. Their masked image errors fall sharply, and the front/left-profile become more recognizable, but the right profile excluded from color fitting barely changes (0.1534→0.1528) and remains blotchy; its prior hair geometry also saw that photo. Renderer execution is `TECHNICAL_CHECK_PASSED`, fitted-view appearance is `PARTIAL_SUCCESS`, and cross-view/full-head promotion is `REJECT`. The next gate is coherent surface/occlusion evidence with a genuinely withheld profile, not more steps on this rejected point cloud.
- Experiment 0173 finds direct hair-mask voting on the current bare head unsound: 22.6% of 5,429 multiply observed points have conflicting hair/skin labels. The seemingly low 3.4% front-surface conflict is mostly skin; among front points with any hair evidence, 44.2% conflict, versus 69.5% at the rear. This is expected under hair occlusion and possible model/camera error, not a camera diagnosis. Direct mask-to-scalp label fusion is `REJECT`; the next route needs view-dependent hairline contours and a separate volume hypothesis.
- Experiment 0172 tests two explicitly swept scalp-to-bun hair candidates on the unchanged head and five locked cameras. Exact-camera rendering passes, but the first looks like a helmet, broom and ball, while the second has a hard hairline, left crown overshoot and visible cylindrical connector. Both are `REJECT`; hand-authored cap/strand/tube variants stop here. Full-head likeness and camera acceptance remain `UNVERIFIED`.
- Experiment 0171 finds a real but limited three-view photo-color association: 971 same-surface samples have median Lab gap 14.625 versus 29.730 shuffled and 20.330 after 5 px oblique shifts. A single bounded nose-depth photo fit improves its training pair by only 0.024 Lab units and worsens both held right-oblique comparisons, so the candidate is `REJECT`. No formal texture or camera/default promotion follows.
- Experiment 0170 extends the one 184-edge open neck boundary and renders all five real views. It removes the immediate floating cut but creates an unnatural straight tube and intersects the apparent shirt/shoulder region; visual/default promotion is `REJECT`. The prior local MV-HRN side artifact is more incomplete, but differing render and camera setups prevent a fair general ranking. Next identity work must address interior face anatomy, not uniform neck extrusion.
- Experiment 0169 transfers six pre-frozen, AI-assisted eye/mouth pixel readings to the current cameras without fitting them. Third-view median/p95 are 4.559/7.218 px with positive depth, but declared reading radii are 4–6 px and the set lacks nose, chin and pure profiles. Camera acceptance remains `UNVERIFIED`; it is not a 3 px gate pass or a visible reconstruction improvement.
...
- Input rights and image-set validation: implemented; missing consent, provenance, license, file hash, view coverage, resolution, or clarity evidence is `REJECT`. Passing these checks does not prove shared rigid capture.
